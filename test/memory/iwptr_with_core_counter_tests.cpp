#include <arba/core/memory/intrusive_weak_ptr.hpp>
...
{
    core::intrusive_weak_ptr<data_with_ircnts> iptr;
...
TEST(iwptr_with_core_counter_tests, test_iwptr_constructor_sptr)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        ASSERT_EQ(isptr.get(), data_ptr);
        ASSERT_EQ(isptr->use_count(), 1);
        ASSERT_TRUE(valid);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
...
TEST(iwptr_with_core_counter_tests, test_iwptr_constructor_sptr_u)
{
    bool valid = false;
    {
        data_with_ircnts* data_ptr = new data_with_ircnts(valid);
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<base_with_ircnts> iwptr(isptr);
        ASSERT_EQ(iwptr.use_count(), 1);
        ASSERT_FALSE(iwptr.expired());
        isptr.release();
        ASSERT_EQ(iwptr.use_count(), 0);
...
TEST(iwptr_with_core_counter_tests, test_iwptr_lock_success)
{
    bool valid = false;
    {
        ...
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        ASSERT_EQ(iwptr.use_count(), 1);
        core::intrusive_shared_ptr<data_with_ircnts> isptr_2 = iwptr.lock();
        ASSERT_TRUE(isptr_2);
...
TEST(iwptr_with_core_counter_tests, test_iwptr_operator_eq_ne)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_2(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_3;
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_4;
...
TEST(iwptr_with_core_counter_tests, test_iwptr_reset)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_null;
        isptr.release();
        ASSERT_TRUE(valid);
...
TEST(iwptr_with_core_counter_tests, test_iwptr_swap)
{
    bool valid = false;
    {
        data_with_ircnts* data_ptr = new data_with_ircnts(valid, "text");
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_alpha(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_data(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_beta;
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_null;
...
TEST(iwptr_with_core_counter_tests, test_iwptr_copy_constructor)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_2(iwptr);
...
TEST(iwptr_with_core_counter_tests, test_iwptr_copy_constructor_u)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<base_with_ircnts> iwptr_2(iwptr);
...
TEST(iwptr_with_core_counter_tests, test_iwptr_move_constructor)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_2(std::move(iwptr));
        ASSERT_EQ(iwptr_2.lock(), isptr);
...
TEST(iwptr_with_core_counter_tests, test_iwptr_move_constructor_u)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<base_with_ircnts> iwptr_2(std::move(iwptr));
        ASSERT_EQ(iwptr_2.lock(), isptr);
        ASSERT_EQ(iwptr.lock().get(), nullptr);
        isptr.release();
        iwptr_2.reset();
...
TEST(iwptr_with_core_counter_tests, test_iwptr_assignement)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr_2;
        iwptr_2 = iwptr;
        ASSERT_EQ(iwptr_2.lock().get(), data_ptr);
        isptr.release();
        iwptr.reset();
...
TEST(iwptr_with_core_counter_tests, test_iwptr_assignement_u)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr(isptr);
        core::intrusive_weak_ptr<base_with_ircnts> iwptr_2;
        iwptr_2 = iwptr;
        ASSERT_EQ(iwptr_2.lock().get(), data_ptr);
        isptr.release();
        iwptr.reset();
...
TEST(iwptr_with_core_counter_tests, test_iwptr_assignement_sptr_u)
{
    bool valid = false;
    {
        data_with_ircnts* data_ptr = new data_with_ircnts(valid);
        core::intrusive_shared_ptr<data_with_ircnts> isptr(data_ptr);
        core::intrusive_weak_ptr<data_with_ircnts> iwptr;
        iwptr = isptr;
        ASSERT_EQ(iwptr.lock(), isptr);
        isptr.release();
        ASSERT_TRUE(iwptr.expired());
