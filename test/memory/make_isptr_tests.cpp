...
TEST(make_isptr_tests, test_make_isptr)
{
    bool valid = false;
    {
        core::intrusive_shared_ptr<data_with_ircnt> iptr = core::make_intrusive_shared_ptr<data_with_ircnt>(valid, "make_iptr");
        ASSERT_TRUE(valid);
...
TEST(make_isptr_tests, test_make_isptr_with_move_arg)
{
    ...
    {
        std::string str("move!");
        core::intrusive_shared_ptr<data_with_ircnt_mc> iptr =
            core::make_intrusive_shared_ptr<data_with_ircnt_mc>(valid, std::move(str));
        ASSERT_TRUE(valid);
        ASSERT_NE(iptr.get(), nullptr);
