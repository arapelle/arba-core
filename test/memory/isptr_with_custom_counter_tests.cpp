...
{
    core::intrusive_shared_ptr<data_with_custom_counter> iptr;
    ASSERT_EQ(iptr.get(), nullptr);
}

TEST(isptr_with_custom_counter_tests, test_iptr_constructor)
{
    bool valid = false;
    {
        data_with_custom_counter* data_ptr = new data_with_custom_counter(valid);
        core::intrusive_shared_ptr<data_with_custom_counter> iptr(data_ptr);
...
TEST(isptr_with_custom_counter_tests, test_iptr_copy_constructor)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_custom_counter> iptr(data_ptr);
        {
            core::intrusive_shared_ptr<data_with_custom_counter> jptr(iptr);
            ASSERT_EQ(iptr.get(), data_ptr);
...
TEST(isptr_with_custom_counter_tests, test_iptr_move_constructor)
{
    ...
    core::intrusive_shared_ptr<data_with_custom_counter> iptr(data_ptr);
    {
        core::intrusive_shared_ptr<data_with_custom_counter> jptr(std::move(iptr));
        ASSERT_EQ(iptr.get(), nullptr);
        ASSERT_EQ(jptr.get(), data_ptr);
