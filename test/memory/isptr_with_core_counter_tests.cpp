...
{
    core::intrusive_shared_ptr<data_with_ircnt> iptr;
...
TEST(isptr_with_core_counter_tests, test_iptr_copy_constructor)
{
    bool valid = false;
    {
        ...
        core::intrusive_shared_ptr<data_with_ircnt> iptr(data_ptr);
        {
            core::intrusive_shared_ptr<data_with_ircnt> jptr(iptr);
            ASSERT_EQ(iptr.get(), data_ptr);
...
TEST(isptr_with_core_counter_tests, test_iptr_move_constructor)
{
    ...
    core::intrusive_shared_ptr<data_with_ircnt> iptr(data_ptr);
    {
        core::intrusive_shared_ptr<data_with_ircnt> jptr(std::move(iptr));
        ASSERT_EQ(iptr.get(), nullptr);
        ASSERT_EQ(jptr.get(), data_ptr);
...
TEST(isptr_with_core_counter_tests, test_iptr_swap)
{
    ...
    core::intrusive_shared_ptr<data_with_ircnt> jptr(other_data_ptr);
    iptr.swap(jptr);
    ASSERT_EQ(iptr.get(), other_data_ptr);
    ASSERT_EQ(iptr->use_count(), 1);
    ASSERT_TRUE(valid);
...
TEST(isptr_with_core_counter_tests, test_iptr_operator_eq)
{
    ...
    data_with_ircnt* data_ptr = new data_with_ircnt(valid);
    core::intrusive_shared_ptr<data_with_ircnt> iptr(data_ptr);
    core::intrusive_shared_ptr<data_with_ircnt> jptr(data_ptr);
    ASSERT_EQ(iptr, jptr);
    ASSERT_EQ(iptr->use_count(), 2);
}
...
TEST(isptr_with_core_counter_tests, test_iptr_set_and_uset)
{
    ...
    bool b_uset = true;
    {
        std::set<core::intrusive_shared_ptr<data_with_ircnt>> iptr_set;
        iptr_set.insert(core::intrusive_shared_ptr<data_with_ircnt>(new data_with_ircnt(b_set)));
        std::unordered_set<core::intrusive_shared_ptr<data_with_ircnt>> iptr_uset;
        iptr_uset.insert(core::intrusive_shared_ptr<data_with_ircnt>(new data_with_ircnt(b_uset)));
    }
    ASSERT_FALSE(b_set);
    ASSERT_FALSE(b_uset);
}
