...
TEST(htow_if_tests, htow_if__endianness_specific__ok)
{
    ...
    const uint16_t swapped_value = core::htow_when(value, cppx::endianness_specific);
...
{
    const uint16_t value = 0x1122;
    const uint16_t swapped_value = core::htow_when(value, cppx::endianness_neutral);
...
{
    const uint16_t value = 0x2211;
    const uint16_t swapped_value = core::wtoh_when(value, cppx::endianness_neutral);
    uint16_t expected_value = 0;
    if constexpr (std::endian::native == std::endian::little)
        expected_value = 0x1122;
    else
