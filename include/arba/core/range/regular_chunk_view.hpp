...
public:
    ...
    explicit regular_chunk_iterator_base_() requires std::default_initializable<iterator_type> {}
...
public:
    ...
    inline regular_chunk_iterator() requires std::default_initializable<iterator_type> {}

    explicit regular_chunk_iterator(iterator_type iter, std::size_t chunk_sz)
        : base_(std::move(iter), chunk_sz)
    {
    }
    ...
    {
        return this->chunk_.begin() == rhs.chunk_.begin()
               && this->chunk_.end() == rhs.chunk_.end();
    }
    [[nodiscard]] inline bool operator!=(const regular_chunk_iterator& rhs) const
    {
        return !(*this == rhs);
    ...
    [[nodiscard]] inline bool operator<(const regular_chunk_iterator& rhs) const requires std::random_access_iterator<iterator_type>
    {
    ...
    regular_chunk_iterator& operator+=(const difference_type& offset) requires std::random_access_iterator<iterator_type>
    {
    ...
    [[nodiscard]] difference_type operator-(const regular_chunk_iterator& other) const requires std::random_access_iterator<iterator_type>
    {
        std::size_t chunk_sz = chunk_size();
        if (chunk_sz != other.chunk_size()) [[unlikely]]
            throw std::runtime_error("Problem when computing regular_chunk_iterator difference: chunk sizes are different.");
        const difference_type diff = this->chunk_.begin() - other.chunk_.begin();
        if (diff % chunk_sz != 0) [[unlikely]]
            throw std::runtime_error("Problem when computing regular_chunk_iterator difference: chunks are not aligned.");
        return diff / chunk_sz;
    }

    [[nodiscard]] decltype(auto) operator[](const difference_type& offset) requires std::random_access_iterator<iterator_type>
    {
        iterator_type iter = std::next(this->chunk_.end(), (offset * chunk_size()));
        return value_type(std::move(iter), std::next(iter, chunk_size()));
    }

    [[nodiscard]] decltype(auto) operator[](const difference_type& offset) const requires std::random_access_iterator<iterator_type>
    {
        iterator_type iter = std::next(this->chunk_.end(), (offset * chunk_size()));
        return value_type(std::move(iter), std::next(iter, chunk_size()));
    }
...
template <typename Iterator>
requires requires(regular_chunk_iterator<Iterator> iter, const typename regular_chunk_iterator<Iterator>::difference_type& offset)
{
    { iter += offset } -> std::same_as<regular_chunk_iterator<Iterator>&>;
}
[[nodiscard]] inline regular_chunk_iterator<Iterator> operator+(regular_chunk_iterator<Iterator> iter,
...
template <typename Iterator>
requires requires(const typename regular_chunk_iterator<Iterator>::difference_type& offset, regular_chunk_iterator<Iterator> iter)
{
    { iter += offset } -> std::same_as<regular_chunk_iterator<Iterator>&>;
}
[[nodiscard]] inline regular_chunk_iterator<Iterator> operator+(const typename regular_chunk_iterator<Iterator>::difference_type& offset,
...
public:
    ...
    regular_chunk_view() requires std::default_initializable<View> {}
    ...
    {
        return iterator(view_.begin(), chunk_size_);
    }
